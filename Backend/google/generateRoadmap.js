...
// REMOVE the top-level skillsArray and skillsText definitions here

exports.getSkillRoadmap = async (req, res) => {
  try {
    const { skill, userProfile } = req.body;

    // MOVE AND FIX THE LOGIC HERE:
    const skillsArray = Array.isArray(userProfile?.skills)
      ? userProfile.skills.map(s => s.name)
      : [];
    
    const skillsText = skillsArray.length > 0 ? skillsArray.join(", ") : "None";
